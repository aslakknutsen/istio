...
func buildAgwDestination(
...
) (*api.RouteBackend, *gatewaycommon.Condition) {
	...
	case gvk.InferencePool.Kind:
		...
		if svc == nil {
			...
			log.Debug(msg)
			invalidBackendErr = &gatewaycommon.Condition{
				Error: &gatewaycommon.ConfigError{
...
func isInvalidBackend(err *gatewaycommon.Condition) bool {
	return err.Reason == gatewaycommon.ConfigErrorReason(gatewayv1.RouteReasonRefNotPermitted) ||
		err.Reason == gatewaycommon.ConfigErrorReason(gatewayv1.RouteReasonBackendNotFound) ||
		err.Reason == gatewaycommon.ConfigErrorReason(gatewayv1.RouteReasonInvalidKind)
}
